module namespace teachers.error = 'content/reports/teachers.error';
...
declare function teachers.error:main( $params ){
    (: чисто для любопытсва :)
    let $href := teachers.error:ссылкаНаИсходныеДанные( $params )
    
    (: данные в формате "похожем-на-RDF" :)
    let $data :=
      $params?_getFileRDF(
         'Utils/adminError.xlsx', (: путь к файлу внутри хранилища :)
    ...
    return
      map{
        'обновления' : <div><h5><center>Проверки данных</center></h5>
